...
BarcodeView.prototype = {
	...
	buildImageUrl : function() {
		...
		var bartext = model.get('text');
		var alttext = model.get('alttext');
		var scale_h = model.get('scale_h');
		var scale_w = model.get('scale_w');
		...
		// TODO 오브젝트가 사라질 때 canvas Object를 제거할 수 있어야 한다.
		if(!this.cvs) {
		...
		try {
			...
			var bm = bw.bitmap();
			return bm.error(this.cvs, rotation);
		}

		return bw.bitmap().draw(this.cvs, rotation);
	},
